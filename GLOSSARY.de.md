...
| **Filter / Sortierung** | `--filter` / `--sort` | Auswahl im Stil eines Kendo-Grids. Filter: `FilterName~op~'value'~and~…` (oder `~or~`); Sortierung: `FieldKey-asc` oder `FieldKey-desc`. Siehe `mastr filters`. |
