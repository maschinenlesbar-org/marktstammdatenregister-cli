...
import { MastrNetworkError } from "../src/client/errors.js";
  ...
  await withServer(
    ...
    async (baseUrl) => {
      const resp = await nodeHttpTransport({
        method: "GET",
        url: `${baseUrl}/MaStR/Einheit/EinheitJson/GetErweiterteOeffentlicheEinheitStromerzeugung?page=1`,
      });
      assert.equal(resp.status, 200);
      assert.deepEqual(JSON.parse(resp.body.toString("utf8")), {
        path: "/MaStR/Einheit/EinheitJson/GetErweiterteOeffentlicheEinheitStromerzeugung?page=1",
      });
    },
  );
});

test("rejects an unsupported protocol with MastrNetworkError", async () => {
  ...
  await withServer(
    ...
    async (baseUrl) => {
      ...
      await assert.rejects(
        ...
        (err) => err instanceof MastrNetworkError && /timed out after 300ms/.test(err.message),
      );
